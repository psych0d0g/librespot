...
mod gstreamer_sink {
    ...
    impl GstreamerSink {
        pub fn open() -> GstreamerSink {
            gst::init();
            let pipeline_str = r#"appsrc caps="audio/x-raw,format=S8,layout=interleaved,channels=2,rate=44100" block=true max-bytes=4096 name=appsrc0 ! audioconvert ! dvbaudiosink"#;
